...
| `PitchShift`       | `pitch` (0 semitones), `engine` (`"granular"`), `windowSize` (0.1 s, granular only), `stretch` (stretch-engine options)      | `pitch`: `Param<number>`                                                          | `isReady: Cell<boolean>`                                      | granular: `windowSize / 2 * sampleRate` samples, fixed; stretch: reported by the engine, updated as the node reports it |
