...
| `SoundPlayer` / `Voice`                  | FX channel (polyphonic one-shot pads)                                                                                                                    |
| Synth source (`AudioProcessor` subclass) | `SynthSource` retained as a tested, restorable module — not in the current all-stem lineup (the real Bass/Vox stems superseded the generated synth-bass) |
...
2. **6 channels (user-asset driven):** Drums, Bass, Synth 1, Synth 2, Vox = StreamPlayer stems; FX = SoundPlayer (8 one-shot pads, click + keys 1–8). _(Earlier iterations used a generated synth-bass; the user's real Bass + Vox stems superseded it. `SynthSource` is kept as a restorable, tested module but is not in the lineup.)_
