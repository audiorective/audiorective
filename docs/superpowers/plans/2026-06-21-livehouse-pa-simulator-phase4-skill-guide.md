...
9. **★ Special note — route signals by what distance should do to them (spatial audio).** This deserves its own callout box. In a spatial app, split every signal by its relationship to listener distance, and make that split a graph-topology decision, not an afterthought:
   - **Direct/dry** sound goes _through_ the panner → attenuates with distance.
   ...
   - The **wet/dry ratio then becomes the distance cue**: near = mostly dry, far = relatively wetter. If your reverb is an _insert on the post-panner signal_, the wet tracks the dry and the ratio never changes — a subtle bug that "sounds fine" until you move and notice the room never opens up. **Worked example:** exactly this bug in the PA simulator — the reverb was post-panner; the fix was a per-`Channel` pre-panner `auxOut` into a dedicated Mixer `auxBus → convolver → wet`, with the headphone toggle muting room **and** aux. Generalize: _aux/send effects that represent a space belong before distance attenuation; only the dry path attenuates._ Show the before/after graph topology side by side.
10. **Other worked-example pitfalls** (real bugs caught building this app — concrete > abstract):
...
11. **A short checklist** the reader can apply to their own app (one line per step above).
