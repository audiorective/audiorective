 ...
 * root graph — once `ready` resolves. Used by `engine.ts` for the page's
 * singleton, and directly by tests to drive the same setup against an offline
 * context: `const { setup, attach } = createLabSetup(); const engine =
